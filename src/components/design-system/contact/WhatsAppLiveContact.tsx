...
export function WhatsAppLiveContact() {
  const pathname = usePathname() || "/en"; const locale = pathname.startsWith("/es") ? "es" : "en";
  ...
  const c = locale === "es" ? { title: "¡Hola! Soy Richard.", text: "¿Necesitas una página web o una plataforma personalizada?", help: "Con gusto puedo ayudarte.", button: "Chatear por WhatsApp", close: "Cerrar tarjeta de contacto", aria: "Abrir conversación de WhatsApp con Richard" } : { title: "Hi! I'm Richard.", text: "Need a website or custom business platform?", help: "I'm happy to help.", button: "Chat on WhatsApp", close: "Close contact card", aria: "Open WhatsApp conversation with Richard" };
  const open = () => window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`, "_blank", "noopener,noreferrer");
