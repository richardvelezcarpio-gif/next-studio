...
export function BrandFooter({ locale }: { locale: "en" | "es" }) {
  ...
  return <footer className="site-footer"><div className="footer-inner"><div className="footer-about"><p className="footer-brand"><Image src="/images/brand/next-studio-logo.png" alt="NEXT STUDIO — Building Digital Businesses" width={180} height={120}/></p><div className="footer-copy"><p className="footer-owned"><span aria-hidden="true">🇪🇨</span> PROUDLY ECUADORIAN-OWNED</p><p>{copy.description}</p><SocialLinks/></div></div><div className="footer-links"><a href={`${base}/${paths.services}`}>{copy.services}</a><a href={`${base}/${paths.projects}`}>{copy.portfolio}</a><a href={`${base}/${paths.tools}`}>{copy.tools}</a><a href={`${base}/${paths.contact}`}>{copy.contact}</a><a href={`${base}/${paths.privacy}`}>{copy.privacy}</a><a href={`${base}/${paths.terms}`}>{copy.terms}</a></div></div><p className="footer-bottom">© {new Date().getFullYear()} {site.name}. {copy.rights}</p></footer>;
}
