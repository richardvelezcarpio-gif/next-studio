...
const content = {
  en: [
    ...
    ["QR Generator", "Create professional QR codes for free.", "Customize, download and share a QR code for your business.", "../qr-generator"],
  ],
  es: [
    ...
    ["Generador QR", "Crea códigos QR profesionales gratis.", "Personaliza, descarga y comparte un QR para tu negocio.", "../generador-qr"],
  ],
} as const;
  ...
  return <div className={s.grid}>{content[locale].map(([title, benefit, detail, path], index) => {
    ...
    return <article className={s.card} key={path}>
      <button className={s.trigger} type="button" onClick={() => setOpen(expanded ? null : index)} aria-expanded={expanded}>
        <div className={s.preview}>{images[index] ? <Image src={`/images/tools/${images[index]}`} alt="" fill sizes="(max-width: 560px) 100vw, (max-width: 850px) 50vw, 33vw" /> : <QrCode size={62}/>}</div>
        <div className={s.cardBody}><span>0{index + 1}</span><h3>{title}</h3><p>{benefit}</p><ChevronDown className={expanded ? s.up : ""} size={21} /></div>
      </button>
      {expanded && <div className={s.details}><p>{copy.benefit}</p><strong>{detail}</strong><Link href={path.startsWith("../") ? `/${locale}/${path.slice(3)}` : `/${locale}/${base}/${path}`}>{copy.open}<ArrowRight size={16} /></Link></div>}
    </article>;
  })}</div>;
}
