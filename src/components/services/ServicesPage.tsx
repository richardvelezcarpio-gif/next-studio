...
export function ServicesPage({ locale }: { locale: Locale }) {
  ...
  const chatHref = getWhatsAppUrl(locale === "en" ? "Hello, I would like information about creating a website or custom business platform." : "Hola, deseo información para crear una página web o plataforma personalizada para mi negocio.");
  return <><BrandHeader locale={locale}/><main>
    ...
    <section className={s.final}><div className={s.wrap}><h2>{c.final}</h2><div><Link href={contact}>{c.cta}<ArrowRight size={17}/></Link>{socialLinks.whatsapp && <a href={chatHref} target="_blank" rel="noopener noreferrer">{c.chat}</a>}</div></div></section>
