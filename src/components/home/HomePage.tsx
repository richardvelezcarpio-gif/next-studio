...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return <><BrandHeader locale={locale} /><main>
    <section className={styles.hero}><div className={styles.container}><div className={styles.heroCopy}><p className={styles.eyebrow}><Sparkles size={14} /> {content.eyebrow}</p><h1>{content.heroTitle}</h1><p className={styles.heroText}>{content.heroText}</p><div className={styles.heroActions}><Link href={contact} className={styles.primaryButton}>{content.primary}<ArrowRight size={17} /></Link><Link href={`/${locale}/${locale === "en" ? "projects" : "proyectos"}`} className={styles.secondaryButton}>{content.secondary}</Link></div></div><div className={styles.heroVisual}><div className={styles.monitor}><Image src="/images/projects/analytics-platform.png" alt="Business analytics dashboard" fill priority/></div><div className={styles.laptop}><Image src="/images/projects/fashion-commerce.png" alt="Premium business website" fill/></div><div className={styles.tablet}><Image src="/images/projects/workflow-platform.png" alt="Operations dashboard" fill/></div><div className={styles.featureTop}>{locale === "en" ? "AI Automation" : "Automatización con IA"}</div><div className={styles.featureBottom}>{locale === "en" ? "Online Booking" : "Reservas online"}</div></div></div></section>
    ...
    <section className={styles.section}><div className={styles.container}><div className={styles.sectionHeading}><p className={styles.kicker}>{locale === "en" ? "INDUSTRIES" : "INDUSTRIAS"}</p><h2>{locale === "en" ? "Digital solutions shaped for your industry." : "Soluciones digitales creadas para tu industria."}</h2></div><IndustryGallery locale={locale}/></div></section>
    ...
    <section className={styles.trust}><div className={styles.container}><div className={styles.trustCopy}><p className={styles.kicker}>WHY NEXT STUDIO</p><h2>{content.trustTitle}</h2></div><div className={styles.trustList}>{content.trust.map((item) => <p key={item}><Check size={18} />{item}</p>)}</div></div></section>
