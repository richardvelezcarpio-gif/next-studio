...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return <><BrandHeader locale={locale} /><main>
    <section className={styles.hero}><div className={styles.container}><div className={styles.heroCopy}><p className={styles.eyebrow}><Sparkles size={14} /> {content.eyebrow}</p><h1>{content.heroTitle}</h1><p className={styles.heroText}>{content.heroText}</p><div className={styles.heroActions}><Link href={contact} className={styles.primaryButton}>{content.primary}<ArrowRight size={17} /></Link><a href="#work" className={styles.secondaryButton}>{content.secondary}</a></div></div><div className={styles.heroVisual}><Image src="/images/hero/digital-business-ecosystem-premium.png" alt="Connected digital business ecosystem" fill priority sizes="(max-width: 900px) 100vw, 65vw" /></div></div></section>
